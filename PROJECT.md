...
| S5b blur calibration | Strength **calibrated to the ACDC-train pool** by sharpness attenuation, measured **at a common normalized short side** (handles BDD 1280×720 ↔ ACDC 1920×1080) with a **variance-normalized Laplacian** as the primary metric (HF-energy ratio secondary); fit by an explicit **forward curve** on BDD-clear (strength → estimator) with a **Tier 0 identifiability gate** (monotonic, target in range, bound clipping recorded); blur parameters have **their own pre-registered ranges** (no S3 equivalent) and are clipped+logged | 2026-09-20 |
...
| S5b | S5 + pool-calibrated condition-specific blur (ancillary ablation) | | | | | 42 | pre-registered (Tier 0/1 pending) |
