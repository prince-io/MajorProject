...
- **Status:** Baseline ladder **S0 / T1 / T1aug / S1 complete and locked** (official split, 1 seed; relabeled from B0/B1/B1aug/B2 on 2026-09-17). The project follows the agreed **S0–S6 comparative-study** design (§5): all stages train on clear BDD and are evaluated on real ACDC, with **S1 (Ultralytics defaults) as the anchor**. **S0/S1/T1/T1aug locked. Official mAP@50: S2 0.2833, S3 0.2741, S5 0.2939, S5b 0.2759.** S5 is the best BDD-trained stage on the official split (~49% of the S1→ceiling headroom), driven by rain/night and bus/truck; 5-fold is an S2/S3/S5 tie (~0.295); fog regresses. **S5b (S5 + pool-calibrated blur) is a controlled negative — it lands near S3/S2 and below S5, and 5-fold (0.281) is the weakest of all augmented BDD stages; blur is not the lever, appearance calibration is.** Next: **S6 design (base = S5).**
- **Last updated:** 2026-09-20 (S5 trained + full S2-matched eval; S5 results recorded)
- **Owner:** student
...
| S5 definition (revised 2026-09-20) | **S3 structure + target-appearance calibration**: identical operators/ranges to S3; per-channel mean/std (saturation reported as a diagnostic) measured on the pool and matched at generation. One-factor over S3 | 2026-09-20 |
...
| S5 calibration limit | Per-parameter physics inversion (direct and forward-curve/severity) was implemented and tested but is **not identifiable** across the BDD↔ACDC base-domain gap; the calibrated quantity is **appearance**, not operator parameters | 2026-09-20 |
...
| S5b blur order | `weather.apply` → **blur** → `appearance_match` (blur **before** the appearance match), so S5's calibrated per-channel mean/std is preserved and S5b differs from S5 by **blur alone**. Physical order is capture-time blur then global tone | 2026-09-20 |
| S5b blur calibration | Strength **calibrated to the ACDC-train pool** by sharpness attenuation, measured **at a common normalized short side** (handles BDD 1280×720 ↔ ACDC 1920×1080) with a **variance-normalized Laplacian** as the primary metric (HF-energy ratio secondary); fit by an explicit **forward curve** built on the **S5-rendered base** with **native-resolution blur** (revised from pre-registered at Tier 0: the clear-BDD base double-counted S5's own sharpness change), targeting the **absolute ACDC-pool sharpness** and gated by a **Tier 0 identifiability gate** (monotonic, target in range, bound clipping recorded); blur parameters have **their own pre-registered ranges** (no S3 equivalent) and are clipped+logged | 2026-09-20 |
| S5b ranges (frozen at Tier 0) | Fractions of short side — **fog defocus σ [0.001, 0.012], rain motion length [0.0015, 0.030], snow defocus σ [0.0003, 0.008]**, night none; fitted (gated) values: fog **0.00136**, rain **0.00555**, snow **0.00075** | 2026-09-20 |
| S5b fallback rule | If the Tier 0 gate fails, use **preview-chosen strengths inside the same pre-registered ranges**; chosen from the pool / clear BDD only — never official val, never the design split | 2026-09-20 |
| S5b boundary | S5b is an **S5b-only sensor/sharpness-calibration exception** (S5 itself does no frequency/sharpness matching — it matches per-channel appearance); blur is not folded into S5 | 2026-09-20 |
| S5b Tier 1 output | Design-split (400) sensitivity probe writes to tracked **`results/summary/blur_probe/`** (never a stage row, never official val); probe code is `src/analysis/blur_probe.py` | 2026-09-20 |
...
**S3 status (2026-09-19).** Implemented per-experiment (S2 frozen) in `src/synth/{stage_common,weather,build_s3,inspect_s3}.py`. Full generation ~2.5 min (8 workers); inspector PASS: 5,000/5,000 labels byte-identical, conditions exactly 1,250 each, each synthetic differs from its source, 5 low-signal sources exempt (documented). Object-visibility (GT-box local-contrast retention, median): fog 0.52, rain 0.90, snow 1.04, night 0.40; night is the most destructive (11.7% of boxes below 0.3), as expected for the hardest condition. Determinism verified (identical image hashes on rebuild). Paper-facing preview grids at `results/summary/figures/synth_preview_bdd_s3_{fog,rain,snow,night}.png`. **Trained + evaluated 2026-09-20** (80 epochs, best @65; full S2-matched eval): official mAP@50 **0.2741**, 5-fold **0.2947 ± 0.0133**, in-domain 0.4799; see the S3 result in §9.

**S3 vs S5 (must not collapse into each other).** S3 uses hand-set weather parameters and no target images (zero-shot). S5 keeps S3's **structure** unchanged and adds **target-appearance calibration** (unlabeled DA): the separation is the appearance-calibration step, not parameter fitting. S3 parameters remain pre-registered and are not tuned from ACDC.

**S5 (revised from pre-registered 2026-09-20).** *Appearance-calibrated weather synthesis.* S3's weather structure is kept **unchanged** (same operators and parameter ranges); S5 adds **target-appearance calibration** — per-channel mean/std measured on the ACDC-train pool are matched at generation (mean saturation is measured and reported as a diagnostic, not force-matched). Same A/B harness, same source list and **same condition-per-source as S3** (`splits/bdd_s3_conditions.csv`), so S5↔S3 is a **one-factor paired** comparison (appearance calibration only). S5 is **unlabeled domain adaptation** (pool statistics only; never official val, never the design split). `src/synth/calibrate.py` computes per-condition appearance distributions (per-channel mean/std, saturation) over the 1,200-image pool, versioned in `results/calibration/synth_stats.json` (pool hash + commit); the builder never recomputes. `src/synth/physics.py` samples S3 operator parameters (identical structure to S3), renders with `weather.apply`, then matches a sampled target per-channel mean/std; saturation is reported as a diagnostic because per-channel mean/std determine it (matching both is over-constrained). Closed-loop QA confirms generated per-channel mean/std match the targets to <~2 (within JPEG effects). **Revision rationale:** per-parameter physics inversion — both direct estimation and forward-curve/severity calibration — was implemented and tested, but is **not identifiable** across the BDD-clear→ACDC base-domain gap (fitted values saturated at the bounds; several statistics were non-monotonic; severity was identifiable for fog only). The calibrated quantity is therefore **appearance**, which is measurable and robust. Recorded as revised-from-pre-registered. Comparisons: S5↔S3 (appearance calibration), S5↔S2 (target-calibrated vs generic photometric), S5↔S4 (physics structure vs Fourier appearance adaptation).

**S5 status (2026-09-20).** Implemented per-experiment (no S2/S3 edits) in `src/synth/{calibrate,physics,build_s5,inspect_s5}.py`; `results/calibration/synth_stats.json` (v3) + `data/yolo/bdd_s5/` (5,000 appearance-calibrated + 5,000 clear A referenced), `splits/bdd_s5_{train.txt,conditions.csv}`, `configs/bdd_s5.yaml` (val = `bdd_src_val.txt`). Inspector PASS: labels 5,000/5,000 byte-identical, conditions exactly 1,250 each, **condition parity with S3**, closed-loop per-channel mean/std diff <~2, determinism verified. Preview grids at `results/summary/figures/synth_preview_bdd_s5_{fog,rain,snow,night}.png`; report `results/summary/synth_report_bdd_s5.txt`. **Trained + evaluated 2026-09-20** (80 epochs, best @66; full S2-matched eval): official mAP@50 **0.2939**, 5-fold **0.2951 ± 0.0148**, in-domain 0.4877; see the S5 result in §9.

**S5b (blur ablation, pre-registered 2026-09-20).** Blur is deliberately **not** folded into S5 (that would make S5↔S3 change two things and muddy the calibration claim). Instead it is an **ancillary one-factor ablation over the calibrated base**: **S5b = S5 rendering + condition-specific blur**, everything else identical (same A/B, same source list and condition-per-source as S3/S5, labels byte-copied, seed 42, 80 epochs, S2-matched eval). Blur mapping: **rain → directional motion blur** aligned with the sampled streak slant; **fog → isotropic defocus**; **snow → mild isotropic defocus**; **night → none** (illumination-limited; blur would only hurt the recall S5 tries to recover). **Pipeline order (pre-registered):** `weather.apply` → **blur** → `appearance_match` — blur is applied **before** the appearance match so S5's calibrated per-channel mean/std is preserved and S5b differs from S5 by **blur alone** (blurring after the match would reduce per-channel std and re-open the S5↔S5b comparison as two factors); this is also the physical order (capture-time blur, then global tone). **Strength is calibrated to the ACDC-train pool** by sharpness attenuation. The estimator (new `src/synth/blur_calibrate.py`; S5's `calibrate.py` untouched) measures per-condition sharpness **at a common normalized short side** to compare BDD 1280×720 with ACDC 1920×1080, using a **variance-normalized Laplacian** as the primary metric plus a high-frequency energy ratio (median over the pool); it builds an explicit **forward curve** (blur strength → estimator) on the **S5-rendered base** (the actual S5b starting point) with blur applied at **native resolution** (revised from pre-registered at Tier 0: using the clear-BDD base double-counted S5's own sharpness change, since S5 rain/snow are ~1.3× sharper than clear BDD), and inverts it to the **absolute ACDC-pool sharpness**, subject to a **Tier 0 identifiability gate** — the curve must be monotonic and the ACDC-pool target must lie inside the curve's range, and any clipping to a pre-registered bound is recorded (upper-bound saturation flags a too-narrow range; the S5 lesson: parameter-level matching across the BDD↔ACDC gap can saturate). A closed-loop check re-applies the fitted blur and re-measures the ratio. Because blur has **no S3 equivalent**, it uses **its own ranges** — isotropic Gaussian σ and directional motion length as fractions of the image short side, clipped and logged — frozen at Tier 0 (fog σ ∈ [0.001, 0.012], rain length ∈ [0.0015, 0.030], snow σ ∈ [0.0003, 0.008]; night none). **Tier 0 (2026-09-20): all three conditions gate PASS, closed-loop absolute error ≤ 0.021, fitted fog 0.00136 / rain 0.00555 / snow 0.00075, no fallback.** **Tier 1 (design split, screening only): test-time blur is ~flat-to-slightly-positive for fog (peak +0.012 at the fitted strength), monotonically harmful for rain (−0.051 at fitted), and beneficial for snow (+0.030 at fitted, +0.040 at 1.5×) — a clear per-condition split (see §10).** **Fallback:** if the gate fails, use **preview-chosen strengths inside the same ranges** (chosen from the pool / clear BDD only). This deliberately crosses the "no frequency matching" line that S5 respects, so S5b is recorded as an **S5b-only sensor/sharpness-calibration exception**. Evidence path: (Tier 0) calibrated-blur preview + object-visibility + sharpness closed-loop; (Tier 1) evaluate the trained S5 model on the **400-image design split** with test-time blur → per-condition sensitivity curve written to tracked **`results/summary/blur_probe/`** (`src/analysis/blur_probe.py`; never official val, never a stage row); (Tier 2) build `data/yolo/s5b` and train only if warranted. Implemented append-only (`src/synth/blur.py` + `blur_calibrate.py` + `build_s5b.py` + `inspect_s5b.py`) composing S5's pipeline (`weather.apply` → blur → `appearance_match`); S5's files untouched. **S5b↔S5 is the only clean one-factor comparison** (S5b↔S3/S1 differ by more than one factor). The outcome feeds **S6a (fixed combination)** and **S6b (condition-aware)**.

**S5b status (2026-09-20).** Tier 0 calibration (`results/calibration/blur_stats.json` v1.0; all gates PASS, closed-loop ≤0.021) and Tier 1 design-split probe (`results/summary/blur_probe/`) are done; the calibration was **revised from pre-registered** to build the forward curve on the S5-rendered base with native-resolution blur (the clear-BDD base double-counted S5's own sharpness). Fitted: fog σ 0.00136, rain length 0.00555, snow σ 0.00075. **Dataset built and inspected:** `data/yolo/bdd_s5b/` = 5,000 blurred B images + 5,000 clear A referenced (manifest `splits/bdd_s5b_train.txt` 10,000; `splits/bdd_s5b_conditions.csv` balanced 1,250/condition; config `configs/bdd_s5b.yaml`, val = `bdd_src_val.txt`). Inspector PASS: labels 5,000/5,000 byte-identical, condition parity with S3, blur present (realized S5b/source: fog 0.33, rain 0.54, snow 0.89, night 1.00 ≈ ACDC targets 0.30/0.57/0.96), appearance closed-loop within JPEG effects, object visibility `frac<0.3` ≤ 0.006. Effective blur: fog σ 0.98 px, rain motion 3.99 px, snow σ 0.54 px, night none. **Trained + evaluated 2026-09-20 (Tier 2, 80 epochs, best @65; full S2-matched eval).** Official val: mAP@50 0.2690 → **0.2759** (+0.0069 over S1) but **below S2 (0.2833) and S5 (0.2939)**; mAP@50-95 0.1601; P 0.535, R 0.278. 5-fold **0.2808 ± 0.0114** — the **weakest of all augmented BDD stages (below S1 0.2863, S2 0.2933, S3 0.2947, S5 0.2951)**; in-domain 0.4802 (−0.0075 vs S5, mild forgetting). **Per weather (official, S5→S5b):** fog **0.463→0.476** (recovers ~half of S5's fog loss, still below S1 0.491), night 0.199→0.177, **rain 0.290→0.265**, snow 0.299→0.298 (flat). **Per class (official, S5→S5b):** **bus 0.255→0.188 (−0.067)**, truck 0.321→0.310, person 0.275→0.258, car/rider/bicycle ~flat; 5-fold every class drops vs S5 (truck −0.035, bus −0.027). **Reading (negative result):** calibrated blur reverses S5's recall-driven gain (R 0.301→0.278, back to a precision-heavy S3-like profile) and specifically undoes the bus/truck gains that carried S5. The Tier 1 probe was directionally right for **rain** (harm) and **fog** (mild help) but over-predicted **snow** — confirming test-time sensitivity ≠ training-time benefit. **Conclusion: the effective lever is global appearance calibration (S5), not sharpness/blur matching; S5 remains the S6a base and blur is excluded from S6a.** S5b stays scoped to the declared S5b-only sensor/sharpness-calibration exception.
...
**Run order:** lock splits → confirm S0/S1 → S2 → S3 → S5 → S5b → S4 → S6a/S6b → optional S6c.
...
src/
  ...
  synth/              # offline synthesis: S2 + S3 + S5 implemented (per-stage, frozen); S5b blur ablation implemented; S6 planned
  analysis/           # S5b Tier 1 design-split blur probe (never a stage row)
...
Official experiment rows (1 seed 42, official ACDC val). **S0 = floor**, **S1 = anchor to beat**, **T1 = in-domain reference**, **T1aug = ceiling** (relabeled 2026-09-17, §5). All 5-fold values are **mean ± sample std (n=5)**, matching `aggregate.py`/`results/summary/summary.json`.
...
| S5 | BDD + calibrated appearance synthesis (S3 structure + target-appearance match) | **0.294** | 0.164 | 0.461 | 0.301 | 42 | **done** |
| S5b | S5 + pool-calibrated condition-specific blur (ancillary ablation) | **0.276** | 0.160 | 0.535 | 0.278 | 42 | **done** |
...
**S3 result — simple weather synthesis (zero-shot), 2026-09-20.** S1 is the exact control (S3 = A clear + B weather; S1 = A + B clear); schedule identical to S1/S2 (80 epochs, best @65, no early stop). Official val: mAP@50 0.2690 → **0.2741** (+0.0051), mAP@50-95 0.1559 → 0.1573 (+0.0014), P 0.461 → **0.532**, R 0.266 → 0.244. 5-fold 0.2863 ± 0.0138 → **0.2947 ± 0.0133** (+0.0084; mAP@50-95 0.1609 → 0.1665 ± 0.0089). In-domain BDD 0.4911 → 0.4799 (−0.0112, mild forgetting). S3 captures ~10% of the S1→T1aug headroom, **below S2 (0.2833) on the official split but statistically tied with S2 on 5-fold (S2 0.2933 ± 0.0161)**. **Per weather (official mAP@50):** fog 0.491→0.489, night 0.193→0.180, **rain 0.244→0.264**, **snow 0.284→0.300**. **Per weather 5-fold mAP@50:** fog 0.476→0.466, night 0.196→0.182, **rain 0.299→0.306**, **snow 0.300→0.306**. **Per class (official):** rider 0.071→**0.117** (best of S1/S2), truck 0.289→0.304, car 0.701→0.693, person 0.270→0.273, bus 0.169→0.167, bicycle 0.113→0.091. **Reading:** hand-set weather structure clears the anchor and ties S2 on 5-fold, but does **not** beat generic photometrics on the official primary metric. The gains are **condition-specific and physical**: **snow is the headline — S3 gives the best BDD-trained snow result (0.300 official, 0.306 5-fold) and closes ~49% of the S1→ceiling snow gap (mAP@50-95 snow 0.159→0.164), with snow recall rising 0.232→0.319 at higher precision**; rain improves too (+0.019 official, +0.008 5-fold). Night (global dimming, no local sources) and fog (constant transmission) regress, and person/bus drop — the overall recall loss (0.266→0.244) lives there, not in rain/snow. **Caveat:** single seed; the S2↔S3 official gap (~0.010) is inside the fold spread (±0.013–0.016), so this is "no advantage over S2", not "harm". Full paper-facing write-up in `paper/results_notes.md`. **Motivates S5 (calibration to measured ACDC-train statistics).**

**S5 result — appearance-calibrated synthesis (unlabeled DA), 2026-09-20.** S3 structure unchanged; S5 adds target-appearance calibration (per-channel mean/std matched to a draw from the ACDC-train pool). Same A/B and condition-per-source as S3; schedule identical (80 epochs, best @66). Official val: mAP@50 0.2690 → **0.2939** (+0.0249), mAP@50-95 0.1559 → 0.1642 (+0.0083), P 0.461 → 0.461, R 0.266 → **0.301**. 5-fold 0.2863 ± 0.0138 → **0.2951 ± 0.0148** (+0.0088; mAP@50-95 0.1670 ± 0.0071). In-domain BDD 0.4911 → 0.4877 (−0.0034). S5 captures **~49% of the S1→T1aug headroom** — the best of any BDD-trained stage (S2 28%, S3 10%) — and is the **best BDD-trained stage on the official split**: +0.0106 over S2, +0.0198 over S3. **Per weather (official mAP@50):** fog 0.491→**0.463** (−0.0286, regression), **night 0.193→0.199**, **rain 0.244→0.290** (~79% of the rain gap), snow 0.284→0.299 (~44%). **Per weather 5-fold mAP@50:** fog 0.476→0.459, night 0.196→0.187, **rain 0.299→0.314**, snow 0.300→0.305. **Per class (official):** **bus 0.169→0.255** (~75% of the bus gap closed), **truck 0.289→0.321** (above the T1aug ceiling 0.313), rider 0.115, bicycle 0.093, car 0.704, person 0.275. **Reading:** calibrating global **appearance** to the target is the first strategy to clearly top the official primary metric, driven by **rain + night and the bus/truck classes** and by a large **recall** gain (0.244 → 0.301). The cost is **fog** (0.463, worst of the BDD-trained stages) and a mAP@50-95 only ~tied with S2. The gain is not uniform and appears **official-split-specific**: on **5-fold S2/S3/S5 are a three-way tie (~0.295)** and the S5↔S3 gap is +0.0004. **Caveat:** single seed; the S3↔S5 official gap (+0.020) exceeds the fold spread (±0.015) but the 5-fold tie means the official advantage is not corroborated across folds. Full paper-facing write-up in `paper/results_notes.md`. **S5 becomes the natural base for S6a/S6b.**

**S5b result — calibrated blur ablation (unlabeled DA, over S5), 2026-09-20.** S5b is the same S5 recipe plus the pool-calibrated, condition-specific blur (rain motion, fog/snow defocus, night none); S5b↔S5 is the only clean one-factor comparison. Official val: mAP@50 0.2939 → **0.2759** (−0.0180), mAP@50-95 0.1642 → 0.1601, P 0.461 → **0.535**, R 0.301 → 0.278. 5-fold 0.2951 ± 0.0148 → **0.2808 ± 0.0114** (−0.0143) — **the weakest augmented BDD stage, below even S1 (0.2863 ± 0.0138)**. In-domain BDD 0.4877 → 0.4802 (−0.0075, mild forgetting). S5b captures ~13% of the S1→ceiling headroom (vs S5's ~49%), landing just above S3 (+0.0018) and below S2 (−0.0074). **Per weather (official mAP@50, S5→S5b): fog 0.463→0.476** (+0.013; recovers ~half of S5's fog regression but still below S1's 0.491), **night 0.199→0.177** (−0.022), **rain 0.290→0.265** (−0.025), snow 0.299→0.298 (flat). **Per weather 5-fold mAP@50:** fog 0.459→0.462, night 0.187→0.186, rain 0.314→0.268, snow 0.305→0.296. **Per class (official, S5→S5b): bus 0.255→0.188** (−0.067), truck 0.321→0.310, person 0.275→0.258, rider 0.115→0.116, car 0.704→0.700, bicycle 0.093→0.084; 5-fold every class drops vs S5 (truck −0.035, bus −0.027, bicycle −0.009, car −0.008, rider −0.006, person −0.002). **Reading:** the blur-driven shift is a **precision-for-recall trade** — S5's headline gain was recall (R 0.266→0.301) concentrated in bus/truck, and blur suppresses exactly that (R → 0.278, a precision-heavy S3-like profile). The mechanism is consistent with the Tier 1 screen: rain harm realized (−0.025), fog mildly positive (+0.013), but **snow benefit did not materialize** (probe predicted +0.030, realized 0.000) — a concrete reminder that test-time sensitivity is not training-time benefit. **Conclusion:** calibrated blur is not the lever; **global appearance calibration (S5) is**, and S5 remains the best BDD-trained stage. S5b is recorded as a **pre-registered controlled negative** that validates the S5 boundary (blur is an S5b-only sensor exception, not part of S5). **Implication for S6:** base = S5, exclude blur; a condition-aware blur arm is not justified (only a small fog gain; snow flat; rain harmful), so S6b should target condition-aware selection/weighting and the S5 fog regression instead. Full paper-facing write-up in `paper/results_notes.md`.
...
| 2026-09-17 | **S0–S6 comparative-study design agreed** (S1 = anchor; S2 photometric, S3 simple weather, S4 FDA, S5 calibrated synthesis, S6a/b/c) *(S5 later revised to appearance calibration, 2026-09-20)* | Peer review (`PLAN.md`): make every method incremental over S1, keep the condition count controlled, and lead with the per-weather comparison |
...
| 2026-09-17 | **S5 = calibrated physics** (parameters fitted to measured ACDC-train statistics) *(superseded 2026-09-20: S5 = S3 structure + target-appearance calibration — see below)* | Keeps S3 (hand-set) and S5 (measured) distinct; uses the unlabeled target legitimately |
...
| 2026-09-20 | **S5b is an S5b-only sensor/sharpness-calibration exception** (S5 itself matches appearance only — no frequency/sharpness matching) | Keeps S5's boundary clean while allowing a measured blur model in the ancillary arm; any S5b claim is scoped to S5b |
...
| 2026-09-20 | **S5b pre-registered (Phase A before any ACDC scoring):** blur applied **before** `appearance_match` (`weather.apply → blur → appearance_match`) so S5b differs from S5 by blur alone; blur strength fitted by a forward-curve sharpness estimator at a **common normalized short side** (variance-normalized Laplacian primary + HF-energy ratio), gated at Tier 0 (monotonic / target-in-range / bound clipping recorded), with a preview-chosen fallback inside the ranges; estimator in new `blur_calibrate.py` (S5's `calibrate.py` untouched); Tier 1 probe in `src/analysis/blur_probe.py`, output to tracked `results/summary/blur_probe/` | Locks the one-factor S5b↔S5 design and the measurement method before results exist; blurring after the match would lower per-channel std and make S5b a two-factor change. Separate calibration module preserves S5's frozen artifact |
| 2026-09-20 | **S5b Tier 0 calibration (revised from pre-registered, before any ACDC scoring):** forward curve rebuilt on the **S5-rendered base** with **native-resolution blur** (the clear-BDD base double-counted S5's sharpness: S5 rain/snow are ~1.3× sharper than clear BDD, so applying the clear-BDD attenuation to S5 under/over-blurred). Ranges frozen: fog σ [0.001,0.012], rain length [0.0015,0.030], snow σ [0.0003,0.008]. All gates PASS; closed-loop abs err ≤ 0.021; fitted fog 0.00136 / rain 0.00555 / snow 0.00075 (no fallback) | Makes the realized S5b sharpness actually match the ACDC target (the clear-BDD base did not); discovered by the Tier 0 preview/closed-loop check, exactly the gate's purpose. `results/calibration/blur_stats.json` v1.0 |
| 2026-09-20 | **S5b Tier 1 design-split probe (screening only):** test-time blur on the 400 design split with the S5 model — fog ~flat/slightly positive (0.512→0.524 at fitted, peak), **rain monotonically harmful** (0.277→0.226 at fitted, →0.197 at 2×), **snow beneficial** (0.280→0.310 at fitted, peak 0.319 at 1.5×). Output `results/summary/blur_probe/` | The screen shows a clear **per-condition split**: calibrated blur is well-matched for fog, harmful for rain at inference, and useful for snow. Because S5b is a single arm applying all three, Tier 2 is a judgement call — test-time sensitivity is not the same as training-time benefit. Tier 2 decision deferred to the user |
| 2026-09-20 | **S5b result (controlled negative):** official mAP@50 **0.2759 vs S5 0.2939 (−0.018)**; 5-fold **0.2808 ± 0.0114 — weakest augmented stage (below S1)**; in-domain 0.4802. Per weather S5→S5b: fog +0.013, night −0.022, rain −0.025, snow flat. Per class: **bus 0.255→0.188**, truck 0.321→0.310; 5-fold every class down | Calibrated blur reverses S5's recall-driven bus/truck gain and does not deliver the probe's snow benefit; **blur is not the lever — appearance calibration (S5) is**. Validates the S5 boundary (blur = S5b-only sensor exception). **S5 stays the S6a base; blur excluded from S6a; no condition-aware blur arm** (only a small fog gain). Paper write-up in `paper/results_notes.md` |
| 2026-09-20 | **S5 parameter inversion found non-identifiable → S5 revised (from pre-registered):** direct per-image physics inversion saturated at bounds; forward-curve calibration was non-monotonic/had targets outside curve range for rain/snow/night; severity was identifiable for fog only | Rendering from BDD-clear scenes means absolute statistics do not track ACDC statistics (cameras/cities/tone), so parameter-level statistic inversion is confounded. The identifiable, robust signal is **global appearance** |
| 2026-09-20 | **S5 redefined = S3 structure + target-appearance calibration:** keep S3 operators/ranges unchanged; measure per-channel mean/std (+ saturation diagnostic) on the pool; match per-channel mean/std at generation. One-factor over S3 | Gives a clean one-factor comparison over S3, is closed-loop by construction, and is a legitimate unlabeled-DA appearance calibration (cf. colour transfer [Reinhard et al., 2001]). Saturation is not force-matched (per-channel mean/std determine it) |
| 2026-09-20 | **S5 implemented + built:** `src/synth/{calibrate,physics,build_s5,inspect_s5}.py` + `results/calibration/synth_stats.json` + `data/yolo/bdd_s5/`; `s5` registered in `stage_common` | Inspector PASS (labels byte-identical, conditions 1,250 each, condition parity with S3, closed-loop mean/std diff <~2), determinism verified. **Awaiting train/eval** |
| 2026-09-20 | **S5 result:** official mAP@50 **0.2939 vs S1 0.2690** (+0.0249) — best BDD-trained stage, +0.0106 over S2, +0.0198 over S3; 5-fold **0.2951 ± 0.0148 ≈ S2/S3**; in-domain 0.4877. Per weather: **rain 0.244→0.290**, **night 0.193→0.199**, snow 0.284→0.299, **fog 0.491→0.463 (regression)**. Per class: **bus 0.169→0.255**, **truck 0.289→0.321** (above ceiling); recall 0.266→0.301 | Appearance calibration tops the **official** primary metric and captures ~49% of the headroom (best BDD-trained), driven by rain/night and bus/truck; but **5-fold is a tie** and **fog regresses**, so the advantage is official-specific and single-seed. S5 is the natural base for S6a/S6b |
...
- **S5 open items:** (a) ~~does appearance-calibrated S5 beat S3/S2?~~ **resolved 2026-09-20: best BDD-trained on the official split, tie on 5-fold**; (b) investigate the **fog regression** (0.463) — likely an appearance-transfer artifact; (c) is per-channel appearance matching too aggressive (strong colour casts, esp. night/rain) and should it be softened; (d) whether a calibration sample-size ablation (design split 100/condition vs pool 300/condition) earns a run.
- **S5b — closed 2026-09-20 (controlled negative, no open items):** (a) ~~pool-calibrated vs preview-chosen~~ **resolved: pool-calibrated forward-curve fit with a Tier 0 gate + preview fallback**; (b) ~~whether Tier 2 was warranted~~ **resolved: trained + evaluated; S5b = 0.2759 official, 0.2808 5-fold — below S5, the weakest augmented stage**; (c) ~~probe tooling~~ **`src/analysis/blur_probe.py`**; (d) ~~Tier 1 output location~~ **`results/summary/blur_probe/`**. Result: blur is not the lever; S5 remains the S6a base and blur is excluded from S6a.
- **S6 open items (next):** S6a = best fixed combination of S2–S5 (base S5, **no blur**); S6b = condition-aware selection/weighting; S6c optional. The S5 **fog regression** (0.463) remains the clearest condition-specific target for S6b, since S5b's only positive was a partial fog recovery (0.476).
- **Class-weighting sensitivity (macro vs micro) — note for future:** mAP is a **macro** mean (every class equal), so rare classes (`rider`/`bus`/`bicycle`) dominate the headline; e.g. dropping `bicycle` from the official mean lifts every run by ~+0.02–0.04 without changing the ranking (S5 0.334 > S2 0.324 > S3 0.311 > S1 0.300; ceiling 0.357). **Keep macro mAP as the primary benchmark number**; if a reviewer questions rare-class dominance, add an **instance-weighted (micro) mAP** or a class-subset view as a clearly-labeled *secondary* metric (never re-score the headline).
