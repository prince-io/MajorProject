...
- **Status:** Baseline ladder **S0 / T1 / T1aug / S1 complete and locked** (official split, 1 seed; relabeled from B0/B1/B1aug/B2 on 2026-09-17). The project follows the agreed **S0–S6 comparative-study** design (§5): all stages train on clear BDD and are evaluated on real ACDC, with **S1 (Ultralytics defaults) as the anchor**. **S0/S1/T1/T1aug locked; S2 0.2833, S3 0.2741 (official mAP@50). S3 ties S2 on 5-fold (0.2947 vs 0.2933) and is the best BDD-trained stage on snow; night/fog expose the limits of hand-set parameters. S5 (appearance-calibrated synthesis) is implemented (dataset built), awaiting train/eval.**
...
| S5 | BDD + calibrated appearance synthesis (S3 structure + target-appearance match) | | | | | 42 | **implemented** (dataset built; awaiting train) |
...
- **S5 open items:** (a) does appearance-calibrated S5 beat S3 (appearance calibration) and S2 (target-calibrated vs generic photometric) on the official/5-fold metrics — the real test; (b) is per-channel appearance matching too aggressive (strong colour casts, esp. night/rain) and should it be softened; (c) whether a calibration sample-size ablation (design split 100/condition vs pool 300/condition) earns a run.
