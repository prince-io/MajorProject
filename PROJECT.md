...
| S5b | S5 + pool-calibrated condition-specific blur (ancillary ablation) | | | | | 42 | dataset ready; training pending |
...
- **S5b open items:** (a) ~~pool-calibrated vs preview-chosen~~ **resolved 2026-09-20 (pre-registered): pool-calibrated forward-curve fit with a Tier 0 identifiability gate + preview fallback**; (b) **open** — whether Tier 2 (training the blur arm) is warranted after the Tier 0/1 probe; (c) ~~probe tooling~~ **resolved: `src/analysis/blur_probe.py`**; (d) ~~Tier 1 output location~~ **resolved: tracked `results/summary/blur_probe/`**. Remaining Tier 0 item: confirm the fitted strengths are not saturated/clipped before trusting them.
