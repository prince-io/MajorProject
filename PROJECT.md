...
src/
  ...
  synth/              # offline synthesis: S2 + S3 + S5 implemented (per-stage, frozen); S5b blur ablation planned; S6 planned
...
- **S5b open items:** (a) confirm the **blur-strength method at Tier 0** (pool-calibrated sharpness vs preview-chosen) given the S5 saturation lesson; (b) whether Tier 2 (training the blur arm) is warranted after the Tier 0/1 probe; (c) how much design-split-probe tooling to build (minimal script vs config + script); (d) Tier 1 output location — `results/analysis/` is gitignored, so use `results/summary/blur_probe/` if the probe should be tracked.
