...
- S5b (**result locked 2026-09-20; code frozen**): `python src/synth/blur_calibrate.py` → `results/calibration/blur_stats.json` (Tier 0 gate), `python src/synth/build_s5b.py --jobs 8` → `data/yolo/bdd_s5b/`, `python src/synth/inspect_s5b.py --dataset-name bdd_s5b`; Tier 1 probe `python src/analysis/blur_probe.py` → `results/summary/blur_probe/`. `synth.blur` is registered in `stage_common.MODULE_BY_STAGE`. Do not edit S5b code (append-only).
