...
- **S5b (calibrated blur ablation, over S5) rules** (`PROJECT.md` §5): ancillary arm, **not** folded into S5. Build on S5's pipeline — `weather.apply` (S3 structure) → `appearance_match` → blur — in `blur.py`/`build_s5b.py` (S5 files untouched). Condition-specific: **rain directional motion** (aligned to streak slant), **fog/snow isotropic defocus**, **night none**. Strength **calibrated to the ACDC-train pool** by sharpness attenuation (new estimator in `calibrate.py`); blur has **its own pre-registered ranges** (no S3 equivalent), clipped + logged. This crosses the "no frequency matching" line S5 respects, so it is an **S5b-only sensor/sharpness-calibration exception**. Evidence: Tier 0 preview → Tier 1 design-split probe (`splits/acdc_design.txt`, never val) → Tier 2 train only if warranted. **S5b↔S5 is the only clean one-factor comparison.** Note: the blur-strength method is to be confirmed at Tier 0 — S5 showed parameter-level matching across the BDD↔ACDC gap can saturate.
