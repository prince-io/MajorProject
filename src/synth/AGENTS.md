...
- **S2** = generic photometric degradation (`photometric.py` + frozen `build_dataset.py`). **S3** = hand-set weather structure (`weather.py` + `build_s3.py`, frozen). **S5** = calibrated physics (`calibrate.py` + `physics.py` + `build_s5.py` + `inspect_s5.py`, designed). **S5b** = calibrated-blur ablation over S5 (`blur.py` + `build_s5b.py`, planned). S6 (combination) later.
...
- **S5 (calibrated physics) rules** (`PROJECT.md` §5): **unlabeled DA** — calibrate on `splits/acdc_pool_unlabeled.txt` statistics only (never official val, never the design split). **Parameter-only**: render by delegating to S3's `weather.apply` (structural parity by construction; do not duplicate the operators). Sample parameters **empirically/bounded** from `results/analysis/synth_stats.json` (never recompute it in the builder; it is versioned with the pool-manifest hash + commit). **Clip fitted values to S3's pre-registered ranges and log every clip**; night vignette is fit then clipped to `[0.10, 0.30]` with the raw distribution reported. Add a **calibration self-test** and a **closed-loop QA** (vs fitted targets and vs the real pool). Reuse **S3's source list and condition-per-source** (`bdd_s3_conditions.csv`) so S5↔S3 is paired parameter-only. Structural changes (row-depth fog, local night, blur) are S6c.
- **S5b (calibrated blur ablation, over S5) rules** (`PROJECT.md` §5): ancillary arm, **not** folded into S5. Compose S5's sampler → `weather.apply` → blur in `blur.py`/`build_s5b.py` (S5 files untouched). Condition-specific: **rain directional motion** (aligned to streak slant), **fog/snow isotropic defocus**, **night none**. Strength **calibrated to the ACDC-train pool** by sharpness attenuation (new estimator in `calibrate.py`); blur has **its own pre-registered ranges** (no S3 equivalent), clipped + logged. This crosses the "no frequency matching" line S5 respects, so it is an **S5b-only sensor/sharpness-calibration exception**. Evidence: Tier 0 preview → Tier 1 design-split probe (`splits/acdc_design.txt`, never val) → Tier 2 train only if warranted. **S5b↔S5 is the only clean one-factor comparison.**
...
- S5 (designed, not yet implemented): `python src/synth/calibrate.py` → `results/analysis/synth_stats.json` (review distributions before building), then `python src/synth/build_s5.py --jobs 8` and `python src/synth/inspect_s5.py --dataset-name bdd_s5`. Register the stage module (`synth.physics`) in `stage_common.MODULE_BY_STAGE`.
- S5b (planned, after S5, before S6): add the blur estimator to `calibrate.py`, then Tier 0 preview, Tier 1 design-split probe (`splits/acdc_design.txt` → `results/analysis/blur_probe/`), and Tier 2 `python src/synth/build_s5b.py --jobs 8` + train/eval only if warranted. Register `synth.blur` in `stage_common.MODULE_BY_STAGE`.
