...
- `src/synth/` experiment code is **append-only**: a stage's generator is frozen once its result is locked (S2 = `photometric.py`/`build_dataset.py`/`inspect_s2.py`; S3 = `weather.py`/`build_s3.py`/`inspect_s3.py`; S5 = `calibrate.py`/`physics.py`/`build_s5.py`/`inspect_s5.py`; **S5b = `blur.py`/`blur_calibrate.py`/`build_s5b.py`/`inspect_s5b.py`, result locked 2026-09-20**). Later stages add their own modules on the shared `synth/stage_common.py` harness. Never overwrite an earlier stage's code; S5's `calibrate.py` stays frozen (the S5b estimator lives in `blur_calibrate.py`).
